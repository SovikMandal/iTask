...
const exportTaskReport = async (req, res) => {
    try {
        ...
        res.setHeader(
            "Content-Disposition",
            `attachment; filename=task_report_${Date.now()}.xlsx`
        );
...
const exportUserReport = async (req, res) => {
    try {
        ...
        const userTasks = await Task.find().populate("assignedTo", "_id name email");
        ...
        res.setHeader(
            "Content-Disposition",
            `attachment; filename=user_report_${Date.now()}.xlsx`
        );
