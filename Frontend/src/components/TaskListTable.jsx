            ...
            {tableData.map((task) => (
                <tr key={task._id} className='border-t border-gray-200'>
                    ...
                    <td className="py-4 px-4">
                        <span className={`px-2 py-1 rounded inline-block whitespace-nowrap ${getStatusColor(task.status)}`}>{task.status}</span>
                    </td>
                    <td className="py-4 px-4">
                        <span className={`px-2 py-1 rounded inline-block whitespace-nowrap ${getPriorityBadgeColor(task.priority)}`}>{task.priority}</span>
                    </td>
